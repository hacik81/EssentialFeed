...
//  EssentialFeedTests
//
//  Created by Sebastian Hat on 08/01/2023.
//
...
extension FeedStoreSpecs where Self: XCTestCase {
    ...
    func assertTheInsertDeliversNoErrorOnNonEmptyCache(on sut: FeedStore, file: StaticString = #file, line: UInt = #line) {
        insert((uniqueImageFeed().local, Date()), to: sut)
        
        let insertionError = insert((uniqueImageFeed().local, Date()), to: sut)
        
        XCTAssertNil(insertionError, "Expect to override cache successfully", file: file, line: line)
    }
